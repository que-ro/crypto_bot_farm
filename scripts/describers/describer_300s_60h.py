...
class Granularity300s60HDescriber(ProductDescriberInterface):

    def __init__(self, date_start: datetime, granularity: int, quote_currency: str, log_lvl=logging.INFO):
        #if granularity = 300
        self.time_needed_for_process = timedelta(hours=60)
        self.date_start = date_start
        self.date_end = date_start + timedelta(hours=60)
        self.granularity = granularity
        self.quote_currency = quote_currency
        self.df_products_description = None
        self.logger = UtilitiesLogger.get_logger(Granularity300s60HDescriber.get_name(), log_lvl=log_lvl)
    ...
    @staticmethod
    def get_name() -> str:
        return 'Granularity300s60HDescriber'
    ...
    def get_df_product_with_description(self) -> object:
        ...
        for currency_pair in list_currency_pairs:
            ...
            for i in range(3):
                date_start_curr = self.date_start + (timedelta(hours=15) * i)
                date_end_curr = date_start_curr + timedelta(hours=15)

                product_historic_rates = [*product_historic_rates,
                                          *public_client.get_product_historic_rates(currency_pair_id,
                                                                                  start=date_start_curr.isoformat(),
                                                                                  end=date_end_curr.isoformat(),
        ...
        self.df_products_description['date_start_describer'] = self.date_start.isoformat()
    ...
    def get_currency_pair_stat_dict(self, currency_pair_id, df_product_historic_rates):
        ...
        # Update spearman correlation of mean price
        self.update_spearman_correlation(currency_pair_stat_dict, df_product_historic_rates)
    ...
    def update_spearman_correlation(self, currency_pair_stat_dict, df_product_historic_rates):

        self.logger.debug('entering update_spearman_correlation(...)')

        # Get mean price
        df_product_historic_rates['mean_price'] = (
                df_product_historic_rates['low']
                + df_product_historic_rates['high']
                + df_product_historic_rates['open']
                + df_product_historic_rates['close']
        ) / 4

        # Get spearman correlation between time and mean price
        corr = df_product_historic_rates.corr(method='spearman').loc['mean_price', 'time']

        # Update stat of currency pair
        currency_pair_stat_dict['spearman_corr_mean_price'] = corr
